...
public class DemographicAnalysisRESTService {
	...
	public Response simpleAnalysis(SimpleAnalysisForm simpleAnalysisForm) {
		...
		if (accountService.hasRole(creator, "PLAN_TESTOWY")) {
			...
			if (!(analysisPoint.getX() > xMin && analysisPoint.getX() < xMax && analysisPoint.getY() > yMin
					&& analysisPoint.getY() < yMax)) {
				BaseResponse response = new BaseResponse();
				response.setResponseStatus(Response.Status.NOT_FOUND);
				response.setMessage("Analiza może zostać wykonana tylko w wyznaczonym obszarze");
				return Response.status(Response.Status.NOT_FOUND).entity(response).build();
			}
		}
	...
	public Response advancedAnalysis(AdvancedAnalysisForm advancedAnalysisForm) {
		...
		default:
			...
			analysis = (AdvancedDemographicAnalysis) (DemographicAnalysisBuilder.advanced()
					.areaType(AreaType.TRAVEL_TIME).travelTimeOrDistance(travelTimeOrDistance).travelType(travelType)
					.ageRange(ageRangeStr).location(location).locationDisplayName(locationDisplayName).creator(creator)
					.name(analysisName).build());
		}
	...
	@Path("/compare")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getComparedAnalyses(@QueryParam("hashes") List<String> hashes,
			@QueryParam("analysisType") String type) {

		Subject currentUser = SecurityUtils.getSubject();
		String username = (String) currentUser.getPrincipal();
		Account account = accountRepository.findByUsername(username);

		List<DemographicAnalysis> accountAnalyses = analysisRepository.getAnalysesDetailsToCompareForAccount(account,
				hashes);
		...
		for (DemographicAnalysis analysis : accountAnalyses) {
			if (analysis instanceof AdvancedDemographicAnalysis) {
				analysesDetailsList.add(new AdvancedDemographicAnalysisDetails((AdvancedDemographicAnalysis) analysis));
			} else if (analysis instanceof SimpleDemographicAnalysis) {
				analysesDetailsList.add(new SimpleDemographicAnalysisDetails((SimpleDemographicAnalysis) analysis));
			}
